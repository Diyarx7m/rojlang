# RojLang interpreter - v1.2
# Kurdish programming language — runs on Python 3
#
# Usage:
...
#
# v1.2 changes:
#   - Strings & comments protected: Kurdish words inside quotes stay untouched
#   - ASCII keyboard mode: nivis / fonksiyon / cewt etc. all work
#     No need for special characters — full English keyboard support
...
# Both proper Kurdish (î ê ş ç û) AND ASCII fallbacks work.
# Longer / more specific patterns first to avoid partial matches.

TRANSLATIONS = [
    # ── functions / control ───────────────────────────────────────
    (r'\bfonksîyon\b',  'def'),
    (r'\bfonksiyon\b',  'def'),       # ASCII: fonksiyon

    (r'\bnivîs\b',      'print'),
    (r'\bnivis\b',      'print'),     # ASCII: nivis

    (r'\bbêje\b',       '_ku_beje'),
    (r'\bbeje\b',       '_ku_beje'),  # ASCII: beje

    (r'\bxwestin\b',    'input'),

    (r'\bhilbijêre\b',  'random.randint'),
    (r'\bhilbijere\b',  'random.randint'),  # ASCII: hilbijere

    # ── conditionals ──────────────────────────────────────────────
    (r'\beger\b',       'if'),
    (r'\byan\b',        'else'),      # 'yan eger' → 'else if' → fixed to elif below
    (r'\bdem\b',        'while'),
    (r'\bvegere\b',     'return'),
    (r'\bbo\b',         'for'),
    (r'\bdi\b',         'in'),
    ...
    # ── loop control ──────────────────────────────────────────────
    (r'\bnîşe\b',       'pass'),
    (r'\bnise\b',       'pass'),      # ASCII: nise

    (r'\bbişkîne\b',    'break'),
    (r'\bbiskine\b',    'break'),     # ASCII: biskine
    ...
    (r'\bne\b',         'not'),
    (r'\bû\b',          'and'),
    (r'\bu\b',          'and'),       # ASCII: u  (careful — only whole word)
    (r'\ban\b',         'or'),

    # ── builtins ──────────────────────────────────────────────────
    (r'\bdirêj\b',      'len'),
    (r'\bdirej\b',      'len'),       # ASCII: direj

    (r'\bcure\b',       'type'),

    (r'\.têxe\b',       '.append'),
    (r'\.texe\b',       '.append'),   # ASCII: .texe
    ...
    # ── v2.0: keyboard / animation aliases ───────────────────────
...
def _ku_beje(*args, **kwargs):
    """Print without newline — bêje / beje"""
    kwargs.setdefault('end', '')
    print(*args, **kwargs)


# ── GUI (tkinter) environment ─────────────────────────────────────
...
def _make_gui_env():
    ...
    # ── new GUI helpers (v2.0) ─────────────────────────────────────

    def _pistad(w, ms, fn):
        """piştî(w, ms, fn) — run fn after ms milliseconds"""
        w.after(int(ms), fn)
    ...
    # klavBiGire(w, "<Control-q>", fn)  → w.bind("<Control-q>", fn)
    # klav_bide(w, "<Return>", fn)      → alias

    def _klav_bi_gire(w, klav, kar):
        """Bind a keyboard event to a function.
        Examples:
          klavBiGire(w, "<Control-q>", lambda e: derkev)
          klavBiGire(w, "<Return>", bikeRun)
          klavBiGire(w, "<Escape>", bigire)
        """
        w.bind(str(klav), kar)
    ...
    def _klav_gişt(w, kar):
        """klavGişt(w, fn) — bind ALL keypress events"""
        w.bind("<KeyPress>", kar)

    # common key name helpers (Kurdish names for key strings)
    _KLAV_CTRL   = "<Control-%s>".__mod__
    _KLAV_ALT    = "<Alt-%s>".__mod__
    _KLAV_SHIFT  = "<Shift-%s>".__mod__

    def _nivisa_enter(kar):
        return "<Return>"     # pass to klavBiGire
    ...
    def _jebike_item(c, item_id):
        """jêbike_item(canvas, id) — delete a canvas item"""
        c.delete(item_id)
...
def _inject_globals(code: str) -> str:
    """
    For every function body, inject  `global x, y, ...`
    for any variable that:
      1. is assigned (x = ...) anywhere in that function body
      2. also exists as a name assigned at module (indent=0) scope

    This lets RojLang code write  nav x = ...  inside functions
    to update global variables naturally, without explicit `global`.
    """
    import re as _re

    lines = code.splitlines(keepends=True)

    # ── Pass 1: find all module-level variable names ──────────────
    # A module-level assignment is any `name =` at indent 0
    # (not def/class/if/for/while/etc.)
    asgn = _re.compile(r'^([\w\u00C0-\u024F\u0600-\u06FF]+)\s*(?:[+\-*/%&|^]?=)(?!=)', _re.UNICODE)
    global_names: set = set()
    for ln in lines:
        s = ln.lstrip()
        if len(ln) - len(s) == 0:          # indent == 0
    ...
    # ── Pass 2: for each function, collect assigned names then inject ─
    result   = []
    i        = 0
    n        = len(lines)
    ...
    while i < n:
        ...
        func_base_indent = len(fm.group(1))   # indent of `def`
        result.append(ln)
        i += 1

        # Determine body indent from first non-empty body line
        ...
        # Collect all lines belonging to this function body
        body_lines = []
        while i < n:
            bln  = lines[i]
            bs   = bln.lstrip()
            bind = len(bln) - len(bs)
            # Empty / comment lines are still part of the body
            if bs == '' or bs.startswith('#'):
                body_lines.append(bln)
                i += 1
                continue
            # If indent <= func_base_indent → we left the function
            if bind <= func_base_indent:
                break
            body_lines.append(bln)
            i += 1

        # Find variables assigned in body that are also global
        needs_global = set()
        for bln in body_lines:
            bs = bln.lstrip()
            # skip comments, nested defs, class
        ...
        # Inject global statement as first line of body
        if needs_global:
            prefix   = ' ' * body_indent
...
def wergerine(source):
    """Translate RojLang → Python.
    String literals and comments are protected — their content is never modified.
    Both Kurdish (î ê ş ç) and ASCII fallbacks (i e s c) are accepted.
    """
    strings_saved  = []
    comments_saved = []
    ...
    # Step 3b — smart global injection
    # "nav x = ..." inside a function body where x exists at module scope
    # Python 3 requires explicit `global x` — we inject it automatically.
...
def bijere(file_path, debug=False):
    # ── Force UTF-8 output on all platforms (fixes Arabic/Kurdish) ──
    ...
    try:
        ...
        # Show original Kurdish line, not translated Python
        ...
        src_lines = source.splitlines()
        # Find deepest frame inside the .ku file
        ku_frame = None
        for frame in tb:
